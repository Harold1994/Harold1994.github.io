...
​	图的最短路径问题主要分为两类,单源最短路径问题和全对最短路径问题。单源最短路径问题指给点单个源点,求其到所有其它顶点之间的最短距离。而全对最短路径问题指所有顶点之间的最短路径问题。此外对于全对最短路径问题,从渐进意义上来看,目前还没有比最好的单源算法更快的算法来解决这一问题。
...
**环路：**最短路径也不能包含权值为正值的环路， 因为只要将环路从路径上删除就可以得到一条源节点和终节点与原来路径相同的权重更小的路径。
...
* Bellman-Ford算法
  ...
  Bellman-Ford 算法采用[动态规划（Dynamic Programming）](http://www.cnblogs.com/gaochundong/p/algorithmic_paradigms.html)进行设计，实现的时间复杂度为 O(V*E)，其中 V 为顶点数量，E 为边的数量。Dijkstra 算法采用[贪心算法（Greedy Algorithm）](http://www.cnblogs.com/gaochundong/p/algorithmic_paradigms.html)范式进行设计，普通实现的时间复杂度为 O(V2)
  ...
  * 设$G=(V,E)$是一个带权重的有向图，权重函数$w：E->R$。设s为源结点，$s—>u->v$是图中的条最短路径，假设图已经进行了一系列的松弛操作，包括对边$（u,v）$的松弛操作。如果对边$(u,v)$进行松弛操作之前的任意时刻有$u.d=dist(s,u)$（即$u.d$为源结点到u结点最短路径，而不再是最短路径估计），那么在该松弛操作之后的所有时刻$v.d=dist(s,v)$. 
  ...
  **算法步骤**
  ...
  ![](https://i.loli.net/2019/03/18/5c8f3e66889c9.png)

  由于图中有 5 个顶点，按照步骤 1 需要遍历 4 次，第一次遍历的结果如下。

  ![2.png](https://i.loli.net/2019/03/18/5c8f3e66e49ff.png)

  第二次遍历的结果如下。

  ![3.png](https://i.loli.net/2019/03/18/5c8f3e671470b.png)
...
弗洛伊德算法，是解决任意两点间的最短路径的一种算法，可以正确处理有向图或负权（但不可存在负权回路）的最短路径问题，同时也被用于计算有向图的传递闭包.
...
因此$ D_{i,j,k}= min(D_{i,j,k-1},D _{i,j,k-1}+D_{k,j,k-1}) $
