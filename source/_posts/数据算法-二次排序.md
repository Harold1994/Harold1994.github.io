...
* 让reducer读取和缓存给定键的所有值,然后对这些值完成in-reducer排序,数据量大时可能导致内存溢出
* 使用MapReduce框架对reducer值排序(这样就不需要再对传入reducer的值进行reducer中的排序),这种方法"会**为自然键增加部分或整个值来创建一个组合键以实现排序目标**",不会造成内存溢出
...
![屏幕快照 2018-10-11 下午10.34.58.png](https://i.loli.net/2018/10/12/5bbff04265a7d.png)
