...
![381412-20161023140159451-370668709.png](https://i.loli.net/2019/03/22/5c94cc2318442.png)
...
确认一个请求究竟发给的是哪个服务端（守护进程），可以通过所收到数据包的目标**端口号**轻松识别。当收到TCP的建立连接请求时，如果目标端口为22，则转给sshd，如果是80则转给httpd。然后，这些守护进程会继续对该连接上的通信传输进行处理。
...
- TCP用于传输层有必要实现可靠传输的情况：面向有连接，具备顺序控制、重发机制等
...
![2018070613375096.png](https://i.loli.net/2019/03/22/5c94cebb18778.png)
...
在TCP中，当发送端的数据到达接收主机时，接收端主机会返回一个已收到消息的通知，这个消息叫确认应答(ACK)，同时也有否认确认应道（NACK）。
...
![20180706142441413.png](https://i.loli.net/2019/03/22/5c94d06ed445e.png)
...
**序列号**是按顺序给发送数据的每一个字节（8位字节）都标上号码的编号，接收端查询接收数据TCP首部中的序列号和数据长度，将自己**下一步应该接收的序号**作为确认应答返送回去。

![20180706143528222.png](https://i.loli.net/2019/03/22/5c94d0db2cdbb.png)
...
![20180706145040438.png](https://i.loli.net/2019/03/22/5c94d17fbe254.png)
...
![20180706150226359.png](https://i.loli.net/2019/03/22/5c94d24fd08d8.png)
...
![2018070615080547.png](https://i.loli.net/2019/03/22/5c94d32f8a494.png)
...
![20180706151500672.png](https://i.loli.net/2019/03/22/5c94d3b917203.png)
...
![20180706151834961.png](https://i.loli.net/2019/03/22/5c94d4017eaaf.png)

其次，考虑一下某个报文段丢失的情况。如下图，接收主机如果收到一个自己应该接收的序号以外的数据时，会针对当前位置收到数据返回确认应答（**不过即使接收端主机收到的包序号并不连续，也不会将数据丢弃而是暂时保存至缓冲区中**）。

![20180706152318181.png](https://i.loli.net/2019/03/22/5c94d46298e20.png)
...
![2018070615323747.png](https://i.loli.net/2019/03/22/5c94d51b86999.png)
...
![20180706153929688.png](https://i.loli.net/2019/03/22/5c94d59889dc1.png)